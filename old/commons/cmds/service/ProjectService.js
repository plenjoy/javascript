...
module.exports = {
    ...
    addOrUpdateAlbum: function(title, obj, eventName) {
        ...
        $.ajax({
            ...
            type: 'get',
            data: 'timestamp=' + timestamp + '&albumId=' + Store.userSettings.albumId + '&albumName=' + title + '&webClientId=' + Store.webClientId + '&autoRandomNum=' + require('UtilMath').getRandomNum()
        }).done(function(result) {
            if (result) {
    ...
    cloneProject:function(obj,oldTitle,newTitle,xml, thumbnailPX, thumbnailPY, thumbnailPW, thumbnailPH){
        ...
        $.ajax({
            ...
            type: 'get',
            data: 'timestamp=' + timestamp + '&albumId=&albumName=' + newTitle + '&webClientId=' + Store.webClientId + '&autoRandomNum=' + require('UtilMath').getRandomNum()
        }).done(function(result) {
            if (result) {
                if ($(result).find('resultData').attr('state') == 'success') {
                    Store.userSettings.albumId = $(result).find('albumId').text() || '';
